...
setwd("C:/Users/leven/OneDrive/Dokumentumok/Dokumentumok/Work/COVID-19/IFR Shift Project/Data")
...
dem.gen <- function(population,year, permillion) {
  pop <- read.table(file = "Population.csv", sep = " ", dec = ".", header = T, stringsAsFactors = F)
  pop <- pop %>%
    filter(Year==year,c_code==population)%>%
    mutate(
      fr.Total = Total / sum(Total),
      fr.Male = Male / sum(Total),
      fr.Female = Female / sum(Total)
    ) %>%
  ...
  if (permillion) {
    pop %>%
      mutate(
        ...
        Male = Male * 1e6 / sum(dem$Total),
        Female = Female * 1e6 / sum(dem$Total)
      )
  }
...
agebased <- function(population = population_def,
  ...
  dem <- dem.gen(population,year,permillion = F)
  ...
  ef <- function(par, func = optFunction, IFR = optIFR) {
    ...
    assign("fx.fm.age", (fx.f.age * filter(dem,fr.Female>=age[1])$fr.Female + fx.m.age * filter(dem,fr.Female>=age[1])$fr.Male) / filter(dem,fr.Female>=age[1])$fr.Total, envir = .GlobalEnv)
    ...
    for (z in 1:(length(ints) - 1)) {
      mort.pred.male.age[z] <<- sum(fx.m.age[intervals$factor == z] * dem$Male[intervals$factor == z] / sum(dem$Male[intervals$factor == z]))
      mort.pred.female.age[z] <<- sum(fx.f.age[intervals$factor == z] * dem$Female[intervals$factor == z] / sum(dem$Female[intervals$factor == z]))
      meanage.male[z] <<- sum(age[intervals$factor == z] * dem$fr.Male[intervals$factor == z] / sum(dem$fr.Male[intervals$factor == z]))
      meanage.female[z] <<- sum(age[intervals$factor == z] * dem$fr.Female[intervals$factor == z] / sum(dem$fr.Female[intervals$factor == z]))
    }
  ...
  fx.age.age.results <<- bind_rows(
    fx.age.age.results,
    tibble(it = 0, age = age[1]:(age[1]+length(fx.m.age)-1), gender = "m", value = fx.m.age),
    tibble(it = 0, age = age[1]:(age[1]+length(fx.f.age)-1), gender = "f", value = fx.f.age),
    tibble(it = 0, age = age[1]:(age[1]+length(fx.m.age)-1), gender = "ms", value = fx.m.age.d.shift[seq(age[1], last(age), by = 0.01) %% 1 == 0])
  )
  ...
  threeModels <- function(mMA, rF) {
    fMA.opts <- tibble(fMA = age) %>%
      mutate(mMA = mMA) %>%
      filter(fMA >= mMA) %>%
      rowwise() %>%
      mutate(
        infected = bind_cols(Age = dem$Age, select(dem, Total, Male, Female) *c(rep(1,nrow(filter(dem,Age<65))),rep(0.7,nrow(filter(dem,Age>=65)))) ) %>% # relative infection attack rate is 0.7 over 65
          mutate(        
            Total = Total * 1e6 / sum(dem$Total),
            Male = Male * 1e6 / sum(dem$Total),
            Female = Female * 1e6 / sum(dem$Total)
          ) %>% #standardize back to 1M infected
          rowwise() %>%
          mutate(
            Male.prot = Male * if (Age < mMA) 1 else rF,
            Female.prot = Female * if (Age < fMA) 1 else rF,
            Total.prot = Male.prot + Female.prot
          ) %>%
          list(),
        mDR = sum(infected$Male.prot * filter(fx.age.age.results, gender == "m")$value) / sum(infected$Male),
        fDR = sum(infected$Female.prot * filter(fx.age.age.results, gender == "f")$value) / sum(infected$Female),
        mDR_fDR = mDR / fDR,
        eqDR.vis = if_else(mDR_fDR >= 0.95 & mDR_fDR <= 1.05, T, F),
        mPD = (sum(infected$Male * filter(fx.age.age.results, gender == "m")$value) - sum(infected$Male.prot * filter(fx.age.age.results, gender == "m")$value)) / sum(infected$Male * filter(fx.age.age.results, gender == "m")$value),
        fPD = (sum(infected$Female * filter(fx.age.age.results, gender == "f")$value) - sum(infected$Female.prot * filter(fx.age.age.results, gender == "f")$value)) / sum(infected$Female * filter(fx.age.age.results, gender == "f")$value),
        mPD_fPD = mPD / fPD,
        eqPD.vis = if_else(mPD_fPD >= 0.95 & mPD_fPD <= 1.05, T, F),
        mYPLL = sum(infected$Male.prot * filter(fx.age.age.results, gender == "m")$value * dem$ex.Male)/sum(infected$Male),
        fYPLL = sum(infected$Female.prot * filter(fx.age.age.results, gender == "f")$value * dem$ex.Female)/sum(infected$Female),
        mYPLL_fYPLL = mYPLL / fYPLL,
        eqYPLL.vis = if_else(mYPLL_fYPLL >= 0.95 & mYPLL_fYPLL <= 1.05, T, F)
      )
    ...
    eqYPLL <- fMA.opts %>%
      rowwise() %>%
      filter(., abs(1 - mYPLL_fYPLL) == min(summarize(fMA.opts, abs(1 - mYPLL_fYPLL)))) %>%
      ungroup() %>%
      slice(1) %>%
      select(fMA, mYPLL, fYPLL, mYPLL_fYPLL, eqYPLL.vis) %>%
      rename(
        eqYPLL.fMA = fMA,
        eqYPLL.mYPLL = mYPLL,
        eqYPLL.fYPLL = fYPLL,
        eqYPLL.mYPLL_fYPLL = mYPLL_fYPLL
      )

    if ((mMA + round(estimate2$p1, 0)) <= last(age)) {
      eqRisk <- fMA.opts %>%
        rowwise() %>%
        ...
        mutate(
          eqRisk.mRisk = filter(fx.age.age.results, gender == "m", age == mMA )$value,
          eqRisk.fRisk = filter(fx.age.age.results, gender == "f", age == eqRisk.fMA )$value,
    ...
    print(paste("Equal Years of Potential Life Lost Model: mMA = ", mMA, "  fMA = ", eqYPLL$eqYPLL.fMA, "  mYPLL/fYPLL = ", eqYPLL$eqYPLL.mYPLL_fYPLL))
    cat("\n")

    return(bind_cols(eqDR, eqPD, eqRisk, eqYPLL))
  }

  # Calculating optimal female protection ages

  minAge.results <<- expand.grid(
    mMA = age[-1],
    # rF=seq(0.1,0.9,by=0.1))%>%
    rF = c(0.1, 1 / 3, 0.5, 2 / 3, 0.9)
  ) %>%
...
plots.models <- function(detailed = F, doc) {
  ...
  if (detailed) {
    s1 <-
      ...
      labs(x = "Minimum age of protection for Male", y = "Minimum age of protection for Female", title = "Equal Death Rate Model") +
      ...
      scale_y_continuous(
        ...
        breaks = seq(from=round_any(age[1], 10, f = floor),to=round_any(last(age), 10, f = ceiling),by=10)
      ) +
      myTheme()
  } else {
    s1 <-
      ...
      geom_line(aes(y = r$eqDR.mDR * scale_s1, group = rF), color = "royalblue", alpha = 0.5) +
      geom_line(aes(y = r$eqDR.fDR * scale_s1, group = rF), color = "firebrick", alpha = 0.5) +
      labs(x = "Minimum age of protection for Male", y = "Minimum age of protection for Female") +
      lims(x = c(age[1], last(age))) +
      scale_y_continuous(
        limits=c(age[1],last(age)),
        sec.axis = sec_axis(trans = ~ . / scale_s1, name = "Overall Death Rate"),
        breaks = seq(from=round_any(age[1], 10, f = floor),to=round_any(last(age), 10, f = ceiling),by=10)
      ) +
      myTheme() +
  ...
  if (detailed) {
    s2 <-
      ...
      labs(x = "Minimum age of protection for Male", y = "Minimum age of protection for Female", title = "Equal Risk Model") +
  ...
  if (detailed) {
    s3 <-
      ...
      labs(x = "Minimum age of protection for Male", y = "Minimum age of protection for Female", title = "Equal Prevented Deaths Model") +
    ...
    s3 <-
      ...
      geom_line(aes(y = r$eqPD.mPD * scale_s3, group = rF), color = "royalblue", alpha = 0.7) +
      geom_line(aes(y = r$eqPD.fPD * scale_s3, group = rF), color = "firebrick", alpha = 0.7) +
      labs(x = "Minimum age of protection for Male", y = "Minimum age of protection for Female") +
      lims(x = c(age[1], last(age))) +
      scale_y_continuous(
        sec.axis = sec_axis(trans = ~ . / scale_s3, name = "Ratio of prevented deaths"),
  ...
  if (detailed) {
    s4 <-
      ggplot(minAge.results, aes(x = mMA)) +
      geom_abline(lty = 2, lwd = 1.2) +
      geom_point(aes(y = r$eqYPLL.fMA, fill = r$eqYPLL.mYPLL_fYPLL, group = rF), size = 3, pch = 21, stroke = 0.01) +
      geom_line(aes(y = r$eqYPLL.mYPLL * scale_s4, group = rF, color = rF), alpha = 0.7) +
      geom_line(aes(y = r$eqYPLL.fYPLL * scale_s4, group = rF, color = rF), alpha = 0.7) +
      labs(x = "Minimum age of protection for Male", y = "Minimum age of protection for Female", title = "Equal YPLL Model") +
      scale_fill_viridis_c(name = "YPLLm/YPLLf", na.value = "transparent", breaks = c(0.8, 1, 1.4, 1.8), labels = c(0.8, 1, 1.4, 1.8), limits = c(0.8, 1.8), option = "C") +
      scale_color_viridis_c(name = "reductionFactor", na.value = "transparent", breaks = c(0, 0.5, 1), labels = c(0, 0.5, 1), limits = c(0, 1), option = "G") +
      scale_y_continuous(
        sec.axis = sec_axis(trans = ~ . / scale_s4, name = "Years of Potntial Life Lost"),
        breaks = seq(from=round_any(age[1], 10, f = floor),to=round_any(last(age), 10, f = ceiling),by=10)
      ) +
      myTheme()
  } else {
    s4 <-
      ggplot(filter(minAge.results, r$eqYPLL.vis, rF == rF.vis), aes(x = mMA)) +
      geom_abline(lty = 2, lwd = 1.2) +
      geom_point(aes(y = r$eqYPLL.fMA), fill = "orchid4", size = 3, pch = 21, stroke = 0.01) +
      geom_line(aes(y = r$eqYPLL.mYPLL * scale_s4, group = rF), color = "royalblue", alpha = 0.7) +
      geom_line(aes(y = r$eqYPLL.fYPLL * scale_s4, group = rF), color = "firebrick", alpha = 0.7) +
      labs(x = "Minimum age of protection for Male", y = "Minimum age of protection for Female") +
      lims(x = c(age[1], last(age))) +
      scale_y_continuous(
        limits=c(age[1],last(age)),
    ...
    s44 <-
      ggplot(filter(minAge.results, r$eqYPLL.vis, rF == rF.vis), aes(x = mMA)) +
      geom_point(aes(y = r$eqYPLL.fMA - mMA), size = 2) +
      labs(y = "Age Shift", title = "Equal Years of Potential Life Model") +
      theme_bw() +
      lims(x = c(age[1], last(age))) +
      theme(
        axis.title.x = element_blank(),
  ...
  if (detailed) {
    ...
    egg::ggarrange(s1, ncol = 1)
    egg::ggarrange(s2, ncol = 1)
    egg::ggarrange(s3, ncol = 1)
    egg::ggarrange(s4, ncol = 1)
    dev.off()
  } else {
    pdf(doc, width = 14, height = 12)
    egg::ggarrange(s11, s1, ncol = 1, heights = c(2, 10))
    egg::ggarrange(s22, s2, ncol = 1, heights = c(2, 10))
    egg::ggarrange(s33, s3, ncol = 1, heights = c(2, 10))
    egg::ggarrange(s44, s4, ncol = 1, heights = c(2, 10))
    dev.off()
  }
}
...
age <- 20:99
bestMethod <- T
population_def <- "GBR_NP"
year_def<-2018
optIFR_def <- "ODriscoll"

agebased(optFunction = "G")

plots.models(detailed = F, doc = "4strategies.pdf")
plots.models(detailed = T, doc = "4strategies_detailed.pdf")

# Save workspace----
 save.image(file = "Results1124.4strat.RData")
