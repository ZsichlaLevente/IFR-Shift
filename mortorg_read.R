...
filenames <- c("mltper_1x1", "fltper_1x1", "Population")
...
Population_db<-Population_db%>%
  left_join(mltper_1x1_db,by=c("Age","Year","c_code"))%>%
  select(c_code,Year,Age,Total,Male,Female,qx,ex)%>%
  rename(mort.Male=qx,
         ex.Male=ex)%>%
  left_join(fltper_1x1_db,by=c("Age","Year","c_code"))%>%
  select(c_code,Year,Age,Total,Male,Female,mort.Male,ex.Male,qx,ex)%>%
  rename(mort.Female=qx,
         ex.Female=ex)

  write.table(file = "Population.csv", Population_db)
